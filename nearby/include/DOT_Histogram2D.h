...
public:
  ...
  double phaseOne(const Histogram2D &A, const Histogram2D &B) {
    ...
    init_coprimes(tau[idxL]);
    idxL++;
    // init_dist_upto(tau[idxL - 1]);

    typedef int FlowType;
    typedef int CostType;

    // Build the graph for min cost flow
    NetSimplex<FlowType, CostType> simplexTwo('E', static_cast<int>(2 * n * n),
                                              0);
    for (size_t i = 0; i < n; ++i)
      for (size_t j = 0; j < n; ++j)
    ...
    double fobj = simplexTwo.totalCost() / simplexTwo.totalFlow();

    PRINT("it: %d, fobj: %f, all: %f, simplex: %f, idxL: %d, num_arcs %ld\n",
          it, fobj, _all, _runtime, idxL, _num_arcs);
