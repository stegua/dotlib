	...
	private:
		...
		public:
			...
			BlockSearchPivotRule(NetSimplexUnit& ns)
				: _source(ns._source), _target(ns._target), _state(ns._state),
				_pi(ns._pi), _in_arc(ns.in_arc), _arc_num(ns._arc_num),
				_dummy_arc(ns._dummy_arc), _next_arc(ns._next_arc) {
