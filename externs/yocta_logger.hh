...
class Logger {
 public:
  ...
  Logger(const std::string& filename)
      : stream(std::fopen(filename.c_str(), "w")), vl(VerbosityLevel::INFO) {}
  ...
  Logger(Logger&& o) noexcept : stream(o.stream), vl(o.vl) {}
  ...
  // Set verbosity level
  void setVerbosityLevel(VerbosityLevel verbosity) { vl = verbosity; }
  // Flush out the buffer stream
  void flush() const { fflush(stream); }
  ...
  void warn(const std::string& format) const {
    if (vl >= VerbosityLevel::WARN) dump("[WARN ]", format);
  }
  ...
  void note(const std::string& format) const {
    if (vl >= VerbosityLevel::NOTE) dump("[NOTE ]", format);
    // Flush right away for errors
    fflush(stream);
  }
  ...
  void info(const std::string& format) const {
    if (vl >= VerbosityLevel::INFO) dump("[INFO ]", format);
  }
